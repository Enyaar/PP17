program poisson
	...
	integer :: loopSize	! Anzahl der Iterationen
	double precision :: accuracy = 10e-8! Fuer Abbruch nach Genauigkeit
	! mpi:
	...
	integer :: exitProd ! Abbruchbedingung
	integer :: i=1, j !Schleifenvariablen
	...
	i = 1
	loopSize = mpi_last + i
	do while ((i - loopSize) < mpi_last)
		call calculate(matrix, mpi_rank, mpi_master, mpi_last, mpi_ierr, loopSize, i, accuracy, exitProd)		
		if (exitProd == 1) then
			if (loopSize == i+mpi_last) then
				loopSize = i
			end if
		else 
			loopSize = i + mpi_last
