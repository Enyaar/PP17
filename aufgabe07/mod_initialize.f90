MODULE initialize
	...
	subroutine allocateMatrix(matrix, rank, numproc, lineCount, matrixSize)
		...
		if ((rank ==  master) .or. (rank == last)) then
			allocate(matrix(matrixSize, (lineCount +1)))
		else
		! Der Rest zwei.
			allocate(matrix(matrixSize, (lineCount +2)))
		endif
	...
	subroutine initializeMatrix(matrix, rank, numproc, lineCount, offset)
		...
		allocate(stepLine(ubound(matrix, 1)))
		matrix = 0.	

		
		do i=1, ubound(matrix,1)-1
			stepLine(i) = REAL(ubound(matrix,1)-i)/REAL(ubound(matrix,1)-1)
		end do

		!print*, offset
		
		! start und ende der belegung (fuer linken und rechten Rand):
		start = offset
		...
		! oberer Rand
		if (rank == master) then
			matrix(:,lbound(matrix,2)) = stepLine(:)
		endif
		
		! links belegen:
		matrix(lbound(matrix,1),lbound(matrix,2):ubound(matrix,2)) = stepLine(start:ende)
		
		! Belegungsvektor umdrehen
		stepLine = 1. - stepLine
		
		! rechts belegen:
		matrix(ubound(matrix,1),lbound(matrix,2):ubound(matrix,2)) = stepLine(start:ende)
		
		! unterer Rand
		if (rank == (numproc - 1)) then
			matrix(:,ubound(matrix,2)) = stepLine(:)
		endif
